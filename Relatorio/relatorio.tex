...
\titulo{Desenvolvimento de um processador baseado em MIPS com kit FPGA}
...
Esse documento tem o propósito de apresentar como ocorreu o planejamento para desenvolver um processador baseado no modelo MIPS com o kit FPGA. Serão apresentadas as escolhas de projeto e definidas as instruções utilizadas, os seus formatos e o caminho de dados que cada uma fará. Serão apresentados o desenvolvimento de cada um dos módulos necessários para o processador, bem como os códigos criados, a integração entre eles e os testes desenvolvidos.

\noindent
\textbf{Palavras-chave}: processador MIPS, kit FPGA, instruções.
\end{resumo}
...
% INTRODUÇÃO
\chapter{Introdução}
No mundo moderno, computadores, celulares, carros modernos, equipamentos médicos, eletrodomésticos e muitos outros aparelhos eletrônicos vêm ganhando cada vez mais espaço na vida das pessoas. Eles realizam as mais diversas tarefas, mas possuem algo em comum: a presença de processadores.

Desse modo, compreender os processadores é uma etapa fundamental para entender o princípio de funcionamento de uma série de objetos do cotidiano. Para isso, é preciso aprender sobre a arquitetura e a organização de computadores, ou seja, como é definida a lógica de funcionamento de um processador e como ela é implementada na prática.

Além disso, uma maneira de se estudar esses conceitos é tomando como referência o MIPS, uma arquitetura de microprocessador, aliado ao kit FPGA. Primeiramente, o MIPS adota um conjunto de instruções mais simples, permitindo uma compreensão mais clara do funcionamento, além de uma implementação mais fácil. Ademais, o kit FPGA é útil para a implementação prática devido à facilidade de testar sinais de entrada e visualizar sinais de saída.

Dessa forma, este documento tem o propósito de apresentar esses e outros conceitos necessários para a implementação de um processador, bem como o planejamento necessário e as escolhas de projeto adotadas.
...
\section{Geral}
O objetivo geral é desenvolver um processador RISC na linguagem Verilog, utilizando o software Quartus e implementá-lo no kit FPGA.
\section{Específicos}
 \begin{enumerate}
   \item Definir quantos registradores serão usados e quantos bits cada um irá armazenar.
   \item Estabelecer os modos de endereçamento que serão aceitos.
   \item Decidir quais serão as instruções do processador, incluindo instruções aritméticas, lógicas, desvios condicionais e incondicionais, acesso à memória (imediato, registrador e deslocamento), movimentação de dados, controle da pilha, acesso aos switches, envio de dados para o display e controle de execução.
   \label{objetivo3}
   \item Determinar quantos bits serão utilizados para cada campo das instruções.
   \item Definir os conjuntos de instruções, os formatos, como será a operação de cada uma e os caminhos de dados.
   \item Desenvolver, usando Verilog, a unidade lógica e aritmética (ULA) e o banco de registradores. Testar a implementação de cada um usando a forma de onda e em seguida, usando esses dois módulos, criar e testar a unidade de processamento.
   \item Criar a memória principal, a memória de instruções e o program counter (PC) e testar cada um desses módulos.
   \item Elaborar a unidade de controle.
   \item Elaborar um conversor de binário para o display de sete segmentos.
   \item Desenvolver o módulo de entrada e saída.
   \item Integrar cada um dos elementos que compõem o processador.
   \item Implementar e testar o processador no kit FPGA.
...
\hline
\texttt{000000} & hlt       & \texttt{010011} & sr        \\
\texttt{000001} & nop       & \texttt{010100} & movr       \\
\texttt{000010} & jmp       & \texttt{010101} & loadd     \\
\texttt{000011} & jal       & \texttt{010110} & loadr     \\
\texttt{000100} & add       & \texttt{010111} & beq       \\
\texttt{000101} & sub       & \texttt{011000} & bne       \\
\texttt{000110} & mul       & \texttt{011001} & blt       \\
\texttt{000111} & div       & \texttt{011010} & bgt       \\
\texttt{001000} & and       & \texttt{011011} & stored    \\
\texttt{001001} & or        & \texttt{011100} & storer    \\
\texttt{001010} & xnor      & \texttt{011101} & movi      \\
\texttt{001011} & addi      & \texttt{011110} & loadi     \\
\texttt{001100} & subi      & \texttt{011111} & in        \\
\texttt{001101} & muli      & \texttt{100000} & pop       \\
\texttt{001110} & divi      & \texttt{100001} & out       \\
\texttt{001111} & andi      & \texttt{100010} & jr        \\
\texttt{010000} & ori       & \texttt{100011} & storei    \\
\texttt{010001} & not       & \texttt{100100} & push      \\
\texttt{010010} & sl        &                 &           \\
\hline
\end{tabular}
\vspace{-0.5em}
...
\hline
Operações aritméticas & \textit{add}, \textit{addi}, \textit{sub}, \textit{subi}, \textit{mul}, \textit{muli}, \textit{div}, \textit{divi} \\
\hline
Operações lógicas & \textit{and}, \textit{andi}, \textit{or}, \textit{ori}, \textit{not}, \textit{xnor}, \textit{sl}, \textit{sr} \\
\hline
Desvios condicionais & \textit{beq}, \textit{bne}, \textit{blt}, \textit{bgt} \\
\hline
Desvios incondicionais & \textit{jmp}, \textit{jr}, \textit{jal} \\
\hline
Acesso à memória & \textit{storei}, \textit{storer}, \textit{stored}, \textit{loadi}, \textit{loadr}, \textit{loadd} \\
\hline
Movimentação de dados & \textit{mov}, \textit{movi} \\
\hline
...
\hline
Controle de execução & \textit{nop}, \textit{hlt} \\
\hline
\end{tabular}
\vspace{-0.5em}
\begin{center}
...
\hline
Formato 3 & \textit{beq}, \textit{bne}, \textit{blt}, \textit{bgt}, , \textit{stored}, \textit{storer} \\
\hline
...
A \autoref{fig:cmdados-f3-desvios} apresenta o caminho de dados para desvios condicionais tomados. Nela, é possível observar o caminho de dados para as instruções beq, bne, blt e bgt.
...
input wire [4:0] SinalULA;   // Codigo de operacaoo
output reg [31:0] SL;        // Saida padrao
output reg SD;               // Saida para desvio
...
module Banco_de_Registradores(clock, reg_leitura1, reg_leitura2, reg_escrita, reg_especial, dado_escrita, dado_especial, SinalBanco, saida1, saida2);
...
input [1:0] SinalBanco;					// Controla a escrita

output [31:0] saida1;					// Dado de saida 1
output [31:0] saida2;					// Dado de saida 2

reg [31:0] banco [63:0];				// Criacao do banco de registradores
...
#(parameter DATA_WIDTH=32, parameter ADDR_WIDTH=6) // 32 bits e 2^6 = 64 linhas
(
...
// Variaveis auxiliares para ficarem com os caracteres
reg [3:0] d0;
reg [3:0] d1;
reg [3:0] d2;
reg [3:0] d3;
...
wire [31:0] Novo_PC;                   // Valor pelo qual o PC sera atualizado 
...
Primeiramente, referente ao planejamento, foi apresentada a fundamentação teórica necessária para se compreender o projeto. Ademais, no desenvolvimento, está presente a escolha da arquitetura MIPS monociclo como referência, os modos de endereçamento, o tamanho das instruções, o total de registradores, os registradores específicos, as instruções que serão implementadas, as operações que cada uma realizará, os nove formatos de instruções, bem como o caminho de dados.

Durante essa etapa, foram encontradas algumas contradições entre a operação realizada pelas instruções storer e storei e o caminho de dados. Além disso, o salto condicional a princípio não estava considerando a posição atual do PC. Ambos os erros já foram corrigidos.

Desse modo, essa etapa foi responsável por guiar os próximos passos com segurança e garantindo que todos os objetivos sejam contemplados.

Em sequência, para o início da implementação, foram desenvolvidos os códigos, bem como a explicação da lógica do funcionamento dos módulos: ULA, banco de registradores, integração ULA com o banco de registradores, memória principal, memória de instruções e PC.

Inicialmente, estava planejado executar as operações de multiplicação e divisão utilizando dois registradores especiais para armazenar o resultado, RH, para a parte alta da multiplicação e RL, para a parte baixa da multiplicação. Porém, constatou-se que isso iria fazer a utilização do processador ser consideravelmente mais complicada. Isso levou a uma mudança para permitir que a parte baixa do resultado pudesse ser escrita em qualquer registrador.

Ainda sobre esse tópico, foi abandonada a ideia de utilizar o registrador específico RH, pois, durante o desenvolvimento da integração entre a ULA e o banco de registradores, foi observado que o tempo de compilação aumentava de cerca de 11 minutos para meia hora ao utilizar duas conexões entre a ULA e o banco de registradores. Sendo assim, o projeto passou a utilizar somente a saída padrão, SL, para essas instruções.

Além disso, para receber e enviar dados para o módulo de entrada e saída, a princípio, seriam utilizados exclusivamente dois registradores especiais, RIN, para receber os dados, e ROUT para enviar. Todavia, isso também poderia dificultar a utilização do processador futuramente, então passou a ser permitido que qualquer registrador desempenhe essas funções.

Um outro desafio encontrado foi o caminho de dados, que originalmente havia sido desenvolvido de um modo muito ineficiente. Desse modo, foi preciso corrigi-lo, tanto para ser mais fácil de compreendê-lo quanto para ser mais simples de implementá-lo.

Além do mais, foram feitos os testes dos módulos desenvolvidos utilizando a forma de onda. Todos os testes feitos têm o funcionamento de acordo com o que foi planejado durante o desenvolvimento, portanto, os códigos desenvolvidos estão funcionando corretamente.

Após essa fase, foram implementados o módulo de controle e feito o início e teste da integração dos módulos. Durante essa etapa, não foram encontradas muitas dificuldades, pois, fazendo a integração aos poucos, tudo funcionou sem problemas.

Para a última etapa, foi desenvolvido o módulo de entrada e saída, integrado todos os módulos e feitos testes no kit FPGA. Durante essa etapa, foram encontradas grandes dificuldades, especialmente para escrever corretamente os valores no display.

Inicialmente foi feita uma tentativa de escrever no display durante todo o tempo da instrução out, porém não estava funcionando e o valor era escrito em um ciclo de clock e perdido no ciclo seguinte. Após muitos testes, a solução encontrada foi escrever no clock somente durante a borda de descida.

Muitas melhorias poderiam ser feitas, incluindo: uma melhor organização do caminho de dados, que poderia ser mais simples e direto; ao invés de escrever em dois registradores ao mesmo tempo no banco, transformar os registradores especiais em buffers que ficariam dentro do módulo onde são usados; teste de mais instruções para garantir ainda mais solidamente o correto funcionamento.
